    ...
    <el-main>
      ...
      <div class="content">
        ...
        <el-table-column
          ...
          >
            <template slot-scope="scope">
              <span>{{ scope.row.recommend_num }}</span>
            </template>
          </el-table-column>
      ...
      >
        ...
        >
          ...
          >
            <el-input
              v-model="editAddForm.recommend_num"
              maxlength="5"
              style="width:60%"
            />
          </el-form-item>
         ...
         <el-form-item label="商品详情图" prop="details_img_id">
                <el-upload
                    action="http://127.0.0.1:8080/api/upload"
...
export default {
  ...
  data() {
    return {
      ...
      dialogRules: {
        ...
        group_num: [
          { required: true, message: "请输入成团数", trigger: "blur" },
        ],
        price: [
          { required: true, message: "请输入商品价格", trigger: "blur" },
        ],
        recommend_num: [
          { required: true, message: "请输入推荐星级", trigger: "blur" },
        ],
       type_id: [
  ...
  methods: {
   ...
   removeDetailUpload(file,fileList){
    ...
    this.ccs.post('/getMcdPage', params).then(rs=>{
      ...
      this.$refs["editAddForm"].validate((valid) => {
        if (valid) {
          ...
          if(this.uoloadFileListCcs.length<3){
           this.$message({
            type: 'warning',
            message: '商品图片最少需要三张，或图片未上传完成,请稍等后再提交！'
            })
              return
            }
          if(this.uoloadDetailFileListCcs.length==0){
            ...
            this.ccs.post('/addMcd', params).then(rs=>{
            if(rs.code!='ccs'){
              this.$message({
                    message: "添加商品成功",
                    type: "success",
               ...
               this.$store.dispatch('LogMessage', "添加商品失败!")
            }
        }).catch(err=>{
          this.$store.dispatch('LogMessage', "添加商品失败!")
          console.log(err)
