...
export default {
    ...
    methods:{
        ...
        login(){
          this.ccs.post('user/login',{user_name: this.loginForm.username,passwork:this.loginForm.password}).then(res=>{
            console.log('-----ccs----'+res)
            if(res.data==0){
                alert('账号密码错误')
            }else{
                ...
                sessionStorage.setItem("teacher_name",res.teacher_name);
                 // 登陆成功后重定向
                    this.$router.push({
